...
This module provides a function :func:`froot` to solve nonlinear systems of
equations
...
where both *x* and the output of *f* can be either scalars or vectors.  The
function operates without attempting to use exact derivative or gradient
information.
...
# Primary function
def froot(f, x0, *a, **kw):
    """Find a root of a nonlinear function or vector function
    ...
    :Inputs:
        ...
        *x0*: :class:`float` | :class:`np.ndarray` (size=*m*)
            Initial guess, scalar or vector
        *x1*: ``class(x0)``
            (Optional) secondary guess, used to scale initial step or bracket
    :Outputs:
        *x*: :class:`float` | :class:`np.ndarray` (size=*n*)
            Solution such that ``f(x)`` is approximately zero
    :Versions:
        * 2010-01-04 ``@ddalle``: First multivariable version
        * 2010-03-11 ``@ddalle``: Added try/catch robustness to 1D part
        * 2010-03-12 ``@ddalle``: Converted data flow to struct objects
        * 2010-03-13 ``@ddalle``: Optimized scalar root search
        * 2010-03-15 ``@ddalle``: Removed all structs, v3.0
        * 2011-04-13 ``@ddalle``: Variable input, v4.0
        * 2018-07-24 ``@ddalle``: Ported to Python (from MATLAB)
    """
    # Process optional args
    if len(a) > 1:
        # Too many inputs
        raise IOError("Too many arguments; function requires 2 or 3 " +
    ...
    # Initial guess type (before conversion)
    cx = x0.__class__
    
    # Ensure array for initial guess
    x0 = np.asarray(x0)
    # Ensure array for secondary guess if applicable
    if x1 is not None:
        # Convert
        x1 = np.asarray(x1)
        # Check size
        if (x1.ndim!=x0.ndim) or (x1.size!=x0.size):
            # Inconsistent
            raise ValueError("Initial and secondary guesses must have " +
    ...
    try:
        ...
        # Could not evaluate
        raise ValueError("Could not evaluate function at initial conditions.")
    ...
    if np.max(np.abs(np.imag(x0.flatten()))) > 0:
        # Complex input
        q_complex = True
    elif np.max(np.abs(np.imag(y0.flatten()))) > 0:
        # Complex initial output
        q_complex = True
    else:
        # No complex values
        q_complex = False
    # Process option
    q_complex = kw.get("complex", kw.get("ComplexSearch", q_complex))
    
    # Reconstruct input function without multidimensional arrays
    if ndy == 0:
        # Scalar output
        if ndx == 0:
            # Scalar input
            g = lambda x: np.asarray(f(x))
        else:
            # Vector input
            g = lambda x: np.asarray(f(np.reshape(x, sx)))
    else:
        # Vector output
        if ndx == 0:
            # Scalar input (?)
            g = lambda x: np.asarray(f(x)).flatten()
        else:
            # Vector input and output
            g = lambda x: np.asarray(f(np.reshape(x, sx))).flatten()
    ...
    imax = kw.get("imax", kw.get("MaxIter", min(1000, max(80, 3*ny))))
    fmax = kw.get("fmax", kw.get("MaxFunEvals", nx+2*imax))
    ...
    while q_cont:
        ...
        if q_bracket:
           # -- Bracketing methods --
            ...
            if np.abs(b-a) > dx / 2 and np.min(np.abs([fa,fb]/fc)) > 0.75:
                # Perform a bisection
                m = (a + b) / 2
