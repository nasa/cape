r"""
:mod:`cape.pyfun.runmatrix`: FUN3D run matrix module
======================================================

This module handles classes and methods that are specific to the list 
of run cases (i.e., the "trajectory").  It is defined in the 
``"RunMatrix"`` section of the master JSON file (e.g. 
:file:`pyFun.json`), and is usually read from a modified 
comma-separated or space-separated text file.

The primary FUN3D state variables are Mach number, angle of attack, and
sidelsip.  To this it is common practice to add a ``"config"`` variable
(which sets the name of the group folder) and a ``"Label"`` which can 
be used to give an extension to the name of a case.  A run matrix using
only these variables could be defined as follows.

    .. code-block:: javascript
    
        "RunMatrix": {
            "Keys": ["mach", "alpha", "beta", "q", "T", "config", 
                     "Label"],
            "File": "inputs/matrix.csv"
        }
        
Then the matrix file :file:`inputs/matrix.csv` could have contents such
as the following.
...
Then the following example API commands could be used to show the case 
names. In this case *fun3d* is the interface to a FUN3D run 
configuration read from a JSON file, and *fun3d.x* is the run matrix.
...
class RunMatrix(cape.runmatrix.RunMatrix):
    r"""Read a simple list of configuration variables
    ...
    :Keyword arguments:
        ...
        *Keys*: :class:`list` of :class:`str` items
            List of variable names, defaults to ``['Mach','alpha',
            'beta']``
        *Prefix*: :class:`str`
    ...
    :Data members:
        ...
        *x.prefix*: :class:`str`
            Prefix to be used in folder names for each case in 
            trajectory
